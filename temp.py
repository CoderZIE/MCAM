...
def multiply2(A,B):
    ...
    ##column 5
    (x20,y20) = ahmad(L[0][3],L[1][4],L[2][5],L[3][6])
    (x21,y21) = AC6G7(L[4][7],x20,0,0)

    ##column 6
    (x0,y0)   = meo(L[0][2],L[1][3],L[2][4],L[3][5])
    (x19,y19) = venka(L[4][6],L[5][7],x0,y20)
    (m5,c5)   = half_adder(x19,y21)

    ##column 7
    (x1,y1) = yang(L[0][1],L[1][2],L[2][3],L[3][4])
    (x2,y2) = momeni(L[4][5],L[5][6],L[6][7],0)
    (x3,y3) = AC6G1(x1,x2,y0,0)
    (m6,c6) = full_adder(x3,y19,c5)

    ##column 8
    (x4,y4) = AC6G2(L[0][0],L[1][1],L[2][2],L[3][3])
    (x5,y5) = AC6G3(L[4][4],L[5][5],L[6][6],L[7][7])
    (x6,y6) = AC6G4(x4,x5,y1,y2)
    (m7,c7) = full_adder(x6,y3,c6)

    ##column 9
    (x7,y7) = ACFGII10(L[1][0],L[2][1],L[3][2],L[4][3])
    (x8,y8) = AC6G6(L[5][4],L[6][5],L[7][6],0)
    (x9,y9) = AC6G7(x7,x8,y4,y5)
    (m8,c8) = full_adder(x9,y6,c7)

    ##column 10
    (x10,y10) = AC6G8(L[2][0],L[3][1],L[4][2],L[5][3])
    (x11,y11) = AC6G9(L[6][4],L[7][5],0,0)
    (x12,y12) = AC6G10(x10,x11,y7,y8)
    (m9,c9)   = full_adder(x12,y9,c8)

    ##column 11
    (x13,y13) = AC6G11(L[3][0],L[4][1],L[5][2],L[6][3])
    (x14,y14) = AC6G12(L[7][4],x13,y10,y11)
    (m10,c10) = full_adder(x14,y12,c9)

    ##column 12
    (x15,y15) = ACFGI1(L[4][0],L[5][1],L[6][2],L[7][3])
    (x16,y16) = half_adder(x15,y13)
    (m11,c11) = full_adder(x16,y14,c10)

    ##column 13
    (x17,y17) = app_compressor(L[5][0],L[6][1],L[7][2],y15)
